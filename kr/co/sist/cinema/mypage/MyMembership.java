...
public class MyMembership extends FileRead {
	...
	/**
	 * '카드가 존재(o) > [멤버십 카드 해지하기], 카드가 존재(x) > [멤버십 카드 등록하기]' 메뉴를 불러오는 메소드
	 */
...
//			System.out.println("                                       ('-' 없이 16자리 숫자를 입력해 주세요.)");
...
//		for (int i=0; i<listMember.size(); i++) {
//			if (listMember.get(i).getId().equals(id)) {
//				listMember.get(i).setCardNum(input);
//				System.out.println();
//				System.out.printf("                                       %s님의 %s가 등록되었습니다. ", listMember.get(i).getName(), listMember.get(i).getCardName());
//				pause();
//				break;
//			} 
//		}
//	}
	...
	/**
	 * ArrayList<Reservation> rs를 내림차순 정렬해서 출력하는 메소드
	 * @param rs : listReservation에서 원하는 정보를 뽑아넣은 ArrayList<Reservation> rs
	 */
	...
	/**
	 * 회원정보(member.dat)에 ArrayList<Member> list의 정보 저장하기
	 */
//	public void saveMember() {
//		try {
//			BufferedWriter writer = new BufferedWriter(new FileWriter(PATH_MEMBER));
//			for (Member m : listMember) {
//				System.out.println("m : " + m);
////				writer.write(String.format("%s■%s■%s■%s■%s■%s■%s■%s■%s■%s■%s■%s■%s■%s■%s■%s■%s■\r\n", m.getId(), m.getPassword(), m.getGender(), m.getName(),  m.getAge(), m.getTel(), m.getAddress(), m.getLastMonthViewCount(), m.getThisMonthViewCount(), m.getPoint(), m.getIsAdmin(), m.getCardNum(), m.getRegistrationDate(), m.getGrade(), m.getIsRegistered(), m.getIssuedDate(), m.getCardName()));
//			}
//			writer.close();
//		} catch (Exception e) {
//			System.out.println(e.toString());
...
//		try {
//			BufferedWriter writer = new BufferedWriter(new FileWriter(PATH_RESERVATION));
//			for (Member m : listMember) {
